...
# ---- Tweakable distance falloff (pixels)
...
func enter(Enemy3):
	...
	if step3_sfx == null:
		...
		# ---- Distance controls
		step3_sfx.attenuation = hear_falloff
		step3_sfx.max_distance = hear_max_distance
	...
	# Keep values in sync if already created earlier
	step3_sfx.attenuation = hear_falloff
	step3_sfx.max_distance = hear_max_distance
...
func physics_update(Enemy3, delta):
	if not Enemy3.player:
		# optional: stop loop if target lost
