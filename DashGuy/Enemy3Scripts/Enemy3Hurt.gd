extends EnemyState3
class_name EnemyHurtState3

const KNOCKBACK_X := 20
const KNOCKBACK_Y := -70
var damage_taken: int = 0

# --- AUDIO: preload hurt sound ---
var HURT_STREAM: AudioStream = preload("res://Sounds/CloakHurt.wav")
var hurt_sfx: AudioStreamPlayer2D

# --- exported volume (editable in Inspector) ---
@export var hurt_volume_db: float = +5.0   # 0 = normal, negative = quieter, positive = louder

func enter(Enemy3):
	print("ENTER NUCK BoB STATE")

	# Stop footsteps if they are playing
	if Enemy3.has_meta("step_sfx"):
		var run_sfx = Enemy3.get_meta("step_sfx")
		if run_sfx and run_sfx is AudioStreamPlayer2D and run_sfx.playing:
			run_sfx.stop()

	# Apply damage
	Enemy3.health -= damage_taken
	if Enemy3.health < 1:
		Enemy3.health = 0
		Enemy3.change_state(EnemyDeathState3.new())
		return

	# Play hurt animation
	Enemy3.anim.play("hurt")
	
		# Knockback direction: opposite of facing
	var direction = -1 if Enemy3.anim.flip_h else 1
	Enemy3.velocity.x = KNOCKBACK_X * direction
	Enemy3.velocity.y = KNOCKBACK_Y

	# Play hurt sound
	if hurt_sfx == null:
		hurt_sfx = AudioStreamPlayer2D.new()
		hurt_sfx.stream = HURT_STREAM
		Enemy3.add_child(hurt_sfx)
	hurt_sfx.pitch_scale = randf_range(0.95, 1.05)
	hurt_sfx.volume_db = hurt_volume_db   # apply inspector value
	hurt_sfx.play()

func physics_update(Enemy3, delta):
	Enemy3.move_and_slide()

	# Wait until hurt anim is finished
	if not Enemy3.anim.is_playing() and Enemy3.health > 0:
		Enemy3.change_state(EnemyIdleState3.new())
